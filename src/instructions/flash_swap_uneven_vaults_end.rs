...
impl FlashSwapUnevenVaultsEnd {
  pub fn instruction(&self, args: FlashSwapUnevenVaultsEndInstructionArgs) -> solana_program::instruction::Instruction {
    self.instruction_with_remaining_accounts(args, &[])
  }
  #[allow(clippy::vec_init_then_push)]
  pub fn instruction_with_remaining_accounts(&self, args: FlashSwapUnevenVaultsEndInstructionArgs, remaining_accounts: &[solana_program::instruction::AccountMeta]) -> solana_program::instruction::Instruction {
    ...
    let mut data = FlashSwapUnevenVaultsEndInstructionData::new().try_to_vec().unwrap();
  ...
  pub fn instruction(&self) -> solana_program::instruction::Instruction {
    let accounts = FlashSwapUnevenVaultsEnd {
          ...
          let args = FlashSwapUnevenVaultsEndInstructionArgs {
                                                              min_repay_amount: self.min_repay_amount.clone().expect("min_repay_amount is not set"),
                                                                  amount_to_leave_to_user: self.amount_to_leave_to_user.clone().expect("amount_to_leave_to_user is not set"),
...
impl<'a, 'b> FlashSwapUnevenVaultsEndCpiBuilder<'a, 'b> {
  ...
  pub fn invoke_signed(&self, signers_seeds: &[&[&[u8]]]) -> solana_program::entrypoint::ProgramResult {
          let args = FlashSwapUnevenVaultsEndInstructionArgs {
                                                              min_repay_amount: self.instruction.min_repay_amount.clone().expect("min_repay_amount is not set"),
                                                                  amount_to_leave_to_user: self.instruction.amount_to_leave_to_user.clone().expect("amount_to_leave_to_user is not set"),
